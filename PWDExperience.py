...
if __name__ == '__main__':
    ...
    """ Plotting Figures """
    debug_plotter = MyPlotlyFigure()  # for debug reasons, its data gets flushed before plotting
    fig_fft = MyPlotlyFigure()  # for FFT results plotting, ex. compare before and after sampling data
    fig_Etotal = MyPlotlyFigure()  # for the total E-field plotting, ex. compare original and reconstructed field
    heatmap = MyPlotlyFigure()  # for the 2D reconstruction of the total E-field
    ...
    ez_kx_y_almost = np.append(
        ez_kx_y[:int(N - N/2 + 1)],
        ez_kx_y[int(N/2 + 1):]
    )
    fig_fft.add_scatter(
        y=np.abs(ez_kx_y_almost),
        x=k_x,
        mode='markers', marker={'size': 5},
        name="with evanescent components",
    )
    ez_kx_y = np.append(
    ...
    fig_fft.add_scatter(
        ...
        x=k_x,
        mode='markers', marker={'size': 5},
        name="without evanescent components",
    )
    fig_fft.matlab_styling()
    fig_fft.update_xaxes(range=[-150, 150])
    fig_fft.update_yaxes(range=[0, 0.02])
    fig_fft.show()
    debug_plotter.data = []
    debug_plotter.add_scatter(y=np.real(ez_kx_y), mode="markers")
    debug_plotter.add_scatter(y=np.imag(ez_kx_y), mode="markers")
    debug_plotter.show()
    ...
    debug_plotter.data = []
    debug_plotter.add_scatter(y=np.real(k_x), mode="markers")
    debug_plotter.add_scatter(y=np.real(k_y), mode="markers")
    debug_plotter.add_scatter(y=np.imag(k_y), mode="markers")
    debug_plotter.show()

    """ Compensating that the sampling is not starting from x=0 coordinate"""
    ez_kx_y_xcomp = ez_kx_y * np.exp(1j * k_x * data_sampled[0, 0])
    debug_plotter.data = []
    debug_plotter.add_scatter(y=np.real(ez_kx_y_xcomp), mode="markers")
    debug_plotter.add_scatter(y=np.imag(ez_kx_y_xcomp), mode="markers")
    debug_plotter.show()
    ...
    debug_plotter.data = []
    debug_plotter.add_scatter(y=np.real(ez_kx_forward), mode="markers")
    debug_plotter.add_scatter(y=np.imag(ez_kx_forward), mode="markers")
    debug_plotter.show()
    ...
    ez_reconstructed_1d = np.zeros(np.shape(x_reconstruct), dtype=np.complex_)
    for k_x_i, k_y_i, ez_kx_forward_i in zip(k_x, k_y, ez_kx_forward):
        ez_reconstructed_1d = ez_reconstructed_1d + \
    ...
    ez_reconstructed_2d = np.zeros(np.shape(x_reconstruct), dtype=np.complex_)
    for k_x_i, k_y_i, ez_kx_forward_i in zip(k_x, k_y, ez_kx_forward):
        ez_reconstructed_2d = ez_reconstructed_2d + \
                              ez_kx_forward_i * np.exp(1j * k_y_i * y_reconstruct) * np.exp(1j * k_x_i * x_reconstruct)
    ...
    heatmap.add_heatmap(
        z=np.abs(ez_reconstructed_2d),
        x=np.real(x_reconstruct),
        y=np.real(y_reconstruct)[:, 0],
        colorscale="rainbow",
        colorbar=dict(
            title="E_z absolute value [V/m]",
            titleside="right",
        ),
        zmin=0,
        zmax=1.75,
    )
    # Add a white empty circle to represent the PEC cylinder in the middle
    heatmap.add_scatter(
        x=np.array([0]),
        y=np.array([0]),
        mode="markers",
    ...
    fig_Etotal.matlab_styling()
    fig_Etotal.show()

    """ Root-Mean-Square Deviation """
    rmsd = (np.real(valid_samping[2, 1::]) - np.real(ez_reconstructed_1d[1:-1:]))**2
    rmsd = np.sqrt(rmsd.sum(axis=0) / rmsd.size)
    nrmsd = rmsd / (np.real(valid_samping[2, 1::]).max(axis=0) - np.real(valid_samping[2, 1::]).min(axis=0))
    print(f"RMSD:{np.format_float_scientific(rmsd, 2)},   NRMSD:{np.format_float_scientific(nrmsd, 2)}")
